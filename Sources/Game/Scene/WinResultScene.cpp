...
void WinResultScene::Initialize(ISceneRequest* pSceneRequest) {
	...
	// BGMを再生する
	ServiceLocater<AudioManager>::Get()->PlayBgm(BgmID::Win, 0, 0.0f);
	// BGMをフェードインさせる
	ServiceLocater<AudioManager>::Get()->FadeBgm(BgmID::Win, 0, 0.9f, 0.0f, 1.0f);
}
	...
	if (!m_wasSelected && m_fadeUI->IsFinished()) {
		...
		if (m_uiObserver->HasNewEvent()) {
			...
			case UIEventID::CharaSelect:
				...
				m_wasSelected = true;
				// BGMをフェードアウトさせる
				ServiceLocater<AudioManager>::Get()->FadeBgm(BgmID::Win, 0, 0.9f, 1.0f, 0.0f);
				break;
				// タイトルに戻る
